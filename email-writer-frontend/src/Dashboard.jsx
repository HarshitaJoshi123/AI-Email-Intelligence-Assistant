...
function Dashboard({ user, onLogout }) {
  ...
  const fetchAnalyses = async () => {
    ...
    try {
      ...
      /*
       * The Dashboard only ever renders once App.jsx has
       * confirmed the user is authenticated via /api/auth/me,
       * so a failure here just means the data request itself
       * failed (session expired mid-session, server hiccup, etc).
       * We surface an error instead of force-redirecting, to
      ...
      setError(
        'Unable to load your email insights.'
      );
  ...
  const overdueCount = useMemo(() => {
    return deadlineEmails.filter(
      (email) =>
        getDeadlineStatus(email).type === 'overdue'
    ).length;
  }, [deadlineEmails]);
  ...
  const aiStatus = useMemo(() => {
    ...
    if (overdueCount > 0) {
      return {
        type: 'alert',
        label: `${overdueCount} overdue ${overdueCount === 1 ? 'item needs' : 'items need'
          } attention`
      };
    }
    ...
    if (actionRequired > 0) {
      return {
        type: 'active',
        label: `${actionRequired} action${actionRequired === 1 ? '' : 's'
          } pending review`
      };
    }
          ...
          <div className="topbar-right">
            ...
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M9 21H5C4.44772 21 4 20.5523 4 20V4C4 3.44772 4.44772 3 5 3H9" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                          ...
                          >
                            ...
                            >
                              ...
                              {email.actionRequired ? (
                                ...
                                <label
                                  className={`action-check ${completed
                                    ? 'done'
                                    : ''
                                    }`}
                                >
                        ...
                        (email) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className={`deadline-icon deadline-icon-${deadlineStatus.type}`}>
                                ◷
                              </div>
